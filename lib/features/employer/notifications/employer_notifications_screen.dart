...
final _empNotifsProvider = FutureProvider.autoDispose<List<_Notif>>((ref) async {
  final res = await ref.watch(dioClientProvider).dio.get(Ep.empNotifications);
...
class _EmpNotifsState extends ConsumerState<EmployerNotificationsScreen> {
  ...
  static const _tabs   = ['all', 'application', 'system'];
  static const _labels = ['All', 'Applications', 'System'];

  Color _borderColor(String type) => switch (type) {
    'application' => AppColors.primary,
    'system'      => AppColors.warning,
    _             => AppColors.success,
  };
  ...
  Future<void> _markAllRead(List<_Notif> notifs) async {
    try {
      ...
      await Future.wait(
        notifs.where((n) => !n.isRead).map((n) => client.dio.post(Ep.empNotifRead(n.id.toString()))),
      );
      ref.invalidate(_empNotifsProvider);
    } catch (_) {}
  }

  @override
  Widget build(BuildContext context) {
    final async = ref.watch(_empNotifsProvider);
    return Scaffold(
      ...
      appBar: UJobAppBar(
        ...
        rightWidget: TextButton(
          ...
          child: Text('Mark all read', style: AppText.label.copyWith(color: AppColors.empPrimary)),
        ),
      ),
      body: Column(children: [
        _FilterBar(
          ...
          labels: _labels,
          selected: _filter,
          activeColor: AppColors.empPrimary,
        ...
        Expanded(
          child: async.when(
            ...
            error: (e, _) => UJobError(
              ...
              onRetry: () => ref.refresh(_empNotifsProvider),
            ),
            data: (notifs) {
                ...
                itemBuilder: (_, i) {
                  ...
                  return _NotifTile(
                    ...
                    onTap: () async {
                      if (!n.isRead) {
                        try {
                          await ref.read(dioClientProvider).dio.post(Ep.empNotifRead(n.id.toString()));
                          ref.invalidate(_empNotifsProvider);
                        } catch (_) {}
                      }
                    },
...
class _NotifTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isRead ? AppColors.surface : AppColors.empPrimary.withValues(alpha: 0.04),
            borderRadius: AppRadius.md,
