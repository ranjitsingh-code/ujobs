...
replacement = "    profileCompleted: int.tryParse(profileData['profile_completed']?.toString() ?? '') ?? int.tryParse(companyData['profile_completed']?.toString() ?? '') ?? int.tryParse(dashData['profile_completed']?.toString() ?? '') ?? 0,"
