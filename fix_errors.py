...
# 2. Replace catch blocks in my_jobs_screen.dart
patterns = [
    ...
    (r"\} catch \(e\) \{\n\s*if \(context.mounted\) UJobToast.error\(context, 'Error', sub: 'Failed to resume job'\);\n\s*\}",
     "} catch (e) {\n                      if (context.mounted) UJobToast.error(context, 'Error', sub: e is DioException ? parseApiError(e) : 'Failed to resume job');\n                    }"),
    ...
    (r"\} catch \(e\) \{\n\s*if \(context.mounted\) UJobToast.error\(context, 'Error', sub: 'Failed to close job'\);\n\s*\}",
     "} catch (e) {\n            if (context.mounted) UJobToast.error(context, 'Error', sub: e is DioException ? parseApiError(e) : 'Failed to close job');\n          }"),
    (r"\} catch \(e\) \{\n\s*if \(context.mounted\) UJobToast.error\(context, 'Error', sub: 'Failed to delete job'\);\n\s*\}",
